<template>
  <ProgressSpinner aria-label="Retrieving questions" v-if="decryptedQuestions.length === 0" />
  <template v-else>
    <Card
      class="w-full sm:w-9/12 m-auto mt-2"
      v-for="(item, index) in decryptedQuestions"
      :key="index"
    >
      <template #header>
        <div class="flex justify-center items-center gap-2">
          <QuizIcon :name="item.tag" class="h-7 w-7" />
          <strong>Question {{ index + 1 }}/{{ decryptedQuestions.length }}</strong>
        </div>
      </template>
      <template #title>
...
const { selectedOptions, decryptedQuestions } = storeToRefs(useQuizStore());
