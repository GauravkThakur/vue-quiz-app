<template>
  <div class="flex flex-col min-h-screen">
    ...
    <main
      v-if="decryptedQuestions.length"
      class="absolute flex-grow top-20 bottom-20 overflow-y-hidden"
    >
      <QuizMain :data="decryptedQuestions" />
    </main>
    <main v-else class="flex justify-center align-center m-auto">
      <ProgressSpinner aria-label="Retrieving questions" />
    </main>
...
const { timeLeft, status, selectedOptions, answers, questions, decryptedQuestions } =
...
const onFinish = () => {
  ...
  answers.value =
    decryptedQuestions.value?.reduce((acc, question, index) => {
